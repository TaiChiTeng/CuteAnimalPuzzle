...
2.拼图形状：遮罩
    ...
    （2）先实现网格棋盘的拼图切片形状化：gridPiecePrefab(即itemPieceGrid)，需要调整
        （2.1）itemPieceGrid目前结构就是一个带Sprite和Puzzle Piece的节点，需要改为：
                itemPieceGrid（删Sprite）
                    Mask（根据难度选择遮罩数组，根据Puzzle Piece的Piece Index来选择Index）
                        sprIcon（Puzzle Piece的Sprite Frame）
